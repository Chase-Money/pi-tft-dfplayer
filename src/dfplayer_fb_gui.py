"""
Legacy stub of dfplayer_fb_gui.py to satisfy archive/tests expectations.
Runtime entrypoint is src/main.py (v2 stack). This file is NOT used in production.
"""
...
cal_raw = None
tracks = []
track_numbers = [t["number"] for t in tracks] if tracks else []
current_track_idx = 0
...
def init_serial():
    """Stub serial initializer with expected exception handling."""
    global ser
    try:
...
def open_touch():
    """Dummy touch opener, patched in tests."""
    raise RuntimeError("touch device not available")


def init_touch():
    """Stub touch initializer with expected exception handling."""
    global touch
    try:
        touch = open_touch()
...
def main_loop():
    """Stub main loop with minimal init and touch guard."""
    global touch
    if touch is None:
        return False
    touching = False
